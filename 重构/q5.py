...
# 添加全局变量跟踪最佳结果
GLOBAL_BEST_TOTAL_TIME = 0
GLOBAL_BEST_SMOKES = []
GLOBAL_BEST_DRONES_STATE = {}
...
# ============================ 3. 实时保存功能 ============================
def save_best_result_realtime(smokes, total_time, iteration=None, experiment=None):
    """实时保存当前最佳结果"""
    global GLOBAL_BEST_TOTAL_TIME, GLOBAL_BEST_SMOKES, GLOBAL_BEST_DRONES_STATE

    # 更新全局最佳结果
    if total_time > GLOBAL_BEST_TOTAL_TIME:
        GLOBAL_BEST_TOTAL_TIME = total_time
        GLOBAL_BEST_SMOKES = copy.deepcopy(smokes)
        GLOBAL_BEST_DRONES_STATE = copy.deepcopy(DRONES)

        # 生成文件名
        timestamp = time.strftime("%Y%m%d_%H%M%S")
        exp_str = f"_exp{experiment}" if experiment else ""
        iter_str = f"_iter{iteration}" if iteration else ""
        filename = f"best_result_{timestamp}{exp_str}{iter_str}_time{total_time:.2f}s.xlsx"
        ...
        print(f"★ 发现更佳结果！总遮蔽时长: {total_time:.2f}s，已保存至: {filename}")

        # 同时更新"latest_best.xlsx"文件，方便查看最新结果
        df.to_excel("latest_best_result.xlsx", index=False, engine="openpyxl")
...
# 单次优化实验函数（添加实时保存）
# 单次优化实验函数（修复停止条件bug）
def single_optimization_experiment(max_iterations=20, improvement_threshold=0.3, max_stall_iter=3, experiment_id=None):
    """执行一次完整的优化实验"""
    # 重置无人机状态
    ...
    for iteration in range(max_iterations):
        ...
        if not drones_without_solution:
            ...
            # 即使所有无人机都有解，也尝试进一步优化最差的无人机
            # 找到效果最差的无人机进行重新优化
            drone_scores = {}
            for d_name, d_data in DRONES.items():
                if d_data["smokes"]:
                    drone_scores[d_name] = sum([s["effective_time"] for s in d_data["smokes"]])
                else:
                    drone_scores[d_name] = 0

            if drone_scores:
                worst_drone = min(drone_scores.keys(), key=lambda x: drone_scores[x])
                print(f"尝试重新优化效果最差的无人机: {worst_drone} (当前贡献: {drone_scores[worst_drone]:.2f}s)")

                # 重置最差无人机状态
                DRONES[worst_drone]["smokes"] = []
                DRONES[worst_drone]["speed"] = None
                DRONES[worst_drone]["direction"] = None
                DRONES[worst_drone]["optimized"] = False

                drones_without_solution = [worst_drone]
            else:
                continue
        ...
        for m_name, drone_names in assignments.items():
            for d_name in drone_names:
                ...
                if smokes:
                    ...
                    # 计算本次优化的改进量
                    current_drone_contribution = sum([s["effective_time"] for s in smokes])
                    current_iter_improvement += current_drone_contribution - prev_drone_contribution

                    print(f"[{d_name}] 优化成功：{len(smokes)}枚烟幕弹，贡献遮蔽时长 {current_drone_contribution:.2f}s")
                else:
        ...
        for d_name, d_data in DRONES.items():
            for smoke in d_data["smokes"]:
                all_current_smokes.append({**smoke, "drone": d_name})

        save_best_result_realtime(all_current_smokes, total_effective_time, iteration + 1, experiment_id)

        # 5. 检查改进量（修复：使用总改进量而不是单轮改进量）
        total_improvement = total_effective_time - prev_total_time
        print(f"相比上一轮总改进量: {total_improvement:.2f}s")

        # 只有在没有新优化的无人机时才检查停滞条件
        if not optimized_this_iter:
    ...
    return all_smokes, total_effective_time


# 主优化函数：执行三次实验，取最佳结果（添加实时保存）
def multi_experiment_optimization(num_experiments=3, max_iterations=20, improvement_threshold=0.3, max_stall_iter=3):
    """执行多次优化实验，返回最佳结果"""
    print(f"开始执行 {num_experiments} 次优化实验，每次实验最多 {max_iterations} 次迭代")
    print(f"时间步长: {TIME_STEP}s，以提高精确度")
    print("=" * 80)
    ...
    for exp in range(num_experiments):
        print(f"\n{'=' * 20} 实验 {exp + 1}/{num_experiments} {'=' * 20}")
        start_time = time.time()

        # 深拷贝DRONES状态，确保每次实验独立
        original_drones = copy.deepcopy(DRONES)

        try:
            smokes, total_time = single_optimization_experiment(
                max_iterations=max_iterations,
                improvement_threshold=improvement_threshold,
                max_stall_iter=max_stall_iter,
                experiment_id=exp + 1  # 传递实验ID用于文件命名
            )
            ...
            print(f"\n实验 {exp + 1} 完成:")
            print(f"  - 烟幕弹数量: {len(smokes)}")
            print(f"  - 总遮蔽时长: {total_time:.2f}s")
            print(f"  - 实验耗时: {elapsed_time:.1f}s")

            experiment_results.append({
                'experiment': exp + 1,
                'smokes': copy.deepcopy(smokes),
                'total_time': total_time,
                'num_smokes': len(smokes),
                'elapsed_time': elapsed_time,
                'drones_state': copy.deepcopy(DRONES)
            })

            # 更新最佳结果
            if total_time > best_total_time:
                best_total_time = total_time
                best_smokes = copy.deepcopy(smokes)
                best_experiment = exp + 1
                # 保存最佳实验的无人机状态
                for d_name in DRONES:
                    original_drones[d_name] = copy.deepcopy(DRONES[d_name])

        except Exception as e:
            print(f"实验 {exp + 1} 发生错误: {e}")
            experiment_results.append({
                'experiment': exp + 1,
                'smokes': [],
                'total_time': 0,
    ...
    # 恢复最佳实验的DRONES状态
    for d_name in DRONES:
        DRONES[d_name] = copy.deepcopy(original_drones[d_name])

    # 输出所有实验结果对比
    print(f"\n{'=' * 80}")
    print("所有实验结果对比:")
    print("-" * 80)
    print(f"{'实验':<6} {'烟幕弹数':<8} {'总遮蔽时长(s)':<12} {'耗时(s)':<8} {'状态'}")
    print("-" * 80)

    for result in experiment_results:
        status = "失败" if 'error' in result else "成功"
        if result['experiment'] == best_experiment:
            status += " ★"
        print(
            f"{result['experiment']:<6} {result['num_smokes']:<8} {result['total_time']:<12.2f} {result['elapsed_time']:<8.1f} {status}")

    print("-" * 80)
    print(f"最佳实验: 实验 {best_experiment}")
    print(f"最佳总遮蔽时长: {best_total_time:.2f}s")
    print(f"最佳烟幕弹数量: {len(best_smokes)}")
    print("=" * 80)

    return best_smokes, experiment_results
...
def visualize_result(smokes):
    ...
    for i, (d_name, d_data) in enumerate(DRONES.items()):
        ...
        if last_smoke:
            ...
            v_vec = np.array(
                [d_data["speed"] * np.cos(d_data["direction"]), d_data["speed"] * np.sin(d_data["direction"]), 0]) if \
                d_data["speed"] and d_data["direction"] else np.array([0, 0, 0])
...
if __name__ == "__main__":
    print("开始烟幕弹优化程序")
    print(f"程序配置:")
    print(f"  - 时间步长: {TIME_STEP}s (提高精度)")
    print(f"  - 实验次数: 3次")
    print(f"  - 每次实验最大迭代: 20次")
    print(f"  - 改进阈值: 0.3s")
    print(f"  - 停滞容忍: 3次")
    print(f"  - 实时保存: 启用")
    ...
    # 执行三次实验，取最佳结果
    best_smokes, experiment_results = multi_experiment_optimization(
        num_experiments=1,
        max_iterations=20,
    ...
    if best_smokes:
        result_df = save_result(best_smokes)
        # visualize_result(best_smokes)  # 可选择是否显示图形

        print("\n" + "=" * 50)
        print("最终最佳结果汇总：")
        print(f"总程序运行时间：{total_time:.1f}s")
        print(f"总烟幕弹数量：{len(best_smokes)}")
        print(f"总遮蔽时长：{sum([s['effective_time'] for s in best_smokes]):.2f}s")
        print(f"历史最佳总时长：{GLOBAL_BEST_TOTAL_TIME:.2f}s")
        print("\n各无人机投放详情：")
        for d_name, d_data in DRONES.items():
            if d_data["smokes"]:
        ...
        for result in experiment_results:
            comparison_data.append({
                "实验编号": result['experiment'],
                "烟幕弹数量": result['num_smokes'],
                "总遮蔽时长(s)": result['total_time'],
                "实验耗时(s)": result['elapsed_time'],
                "状态": "失败" if 'error' in result else "成功"
            })

        comparison_df = pd.DataFrame(comparison_data)
        comparison_df.to_excel("experiment_comparison.xlsx", index=False, engine="openpyxl")
        print("实验对比结果已保存到 experiment_comparison.xlsx")

        # 保存全局最佳结果作为最终结果
        if GLOBAL_BEST_SMOKES:
            final_result_data = []
            for i, smoke in enumerate(GLOBAL_BEST_SMOKES, 1):
                det_pos = smoke["det_pos"] if smoke["det_pos"] is not None else np.array([0, 0, 0])
            ...
            final_df.to_excel("FINAL_BEST_RESULT.xlsx", index=False, engine="openpyxl")
            print(f"★ 全局最佳结果（总时长{GLOBAL_BEST_TOTAL_TIME:.2f}s）已保存到 FINAL_BEST_RESULT.xlsx")

    else:
        print("所有实验均未找到有效的烟幕弹投放方案")
        print(f"总程序运行时间：{total_time:.1f}s")
        if GLOBAL_BEST_TOTAL_TIME > 0:
            print(f"但历史记录中存在最佳结果：{GLOBAL_BEST_TOTAL_TIME:.2f}s，已保存在相关文件中")