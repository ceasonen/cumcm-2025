后三问的思路是基于前两问的，第三问可以在第二问的基础上优化第一枚烟雾弹，抛出以后呢，以第一第二问的相同思路取抛出的地点为起点，就又成了第一、二问的问题，也就是规定了你的速度，但是扔的时间是可以改变的，那么我们就可以基于前两问的思路来求解第二枚烟雾弹应该投放的位置和时间，第三枚同理。这么就变成了一个环环相扣的问题，那么在达成你的策略后，可以通过优化第一枚烟雾弹抛出的时间以及无人机最开始的巡逻速度和飞行方向来进行优化（第二次优化），那么这样这题就变成了两个优化问题。注意，我的第三问代码是基于第二问5.7s的初始数据计算的！你也可以按照你的结果修改！
代码3
